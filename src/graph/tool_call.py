    ...
    if is_tier1_retry:
        ...
        if is_tier2_retry:
            ...
            prompt = (
                ...
                "참고: tool 선택과 조건 채우기는 서로 다른 기준을 따릅니다.\n"
                "1) tool 선택: 이번 발화가 새로운 상품 종류(예: 주택담보대출, 사업자대출 등)를 "
                "명시했다면, 이전 턴에 어떤 tool을 썼는지와 무관하게 반드시 그 새 상품에 맞는 "
                "tool로 전환하세요. 이번 발화가 그 자체로는 모호해서(예: \"왜 그래요?\", \"다시 "
                "한번 봐주세요\") 어떤 상품인지 알 수 없을 때만 이전 대화에서 다루던 tool을 "
                "그대로 사용하세요.\n"
                "2) 조건 채우기: 은행명·신용점수·기간처럼 이번 발화에 언급되지 않은 조건은, "
                "tool을 전환하는 경우라도 이전 대화에서 확인된 값을 그대로 이어받아 채우세요.\n"
                "되묻지 말고 이력을 근거로 판단해 반드시 tool을 호출하세요."
            )
